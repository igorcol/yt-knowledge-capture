...
"Recebi a transcrição bruta. Iniciando protocolo de destilação 80/20. Alinhando neurônios para extração de sinal puro. O conhecimento será injetado agora."
  ...
  try {
    const prompt = `
      Você é o Sintetizador Neural 80/20. Atue como filtro de alta precisão para um empreendedor visionário.
    ...
    return SynthesisSchema.parse(JSON.parse(result.response.text()));
    } catch (error) {
